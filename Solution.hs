...
all_numbers_of_size 1 from = fromList $ [(x, Sum_of_numbers $ toInteger x) | x <- [0..9]]
...
combine begins ends ends_size = 
  sum $
  elems $
  intersectionWith (\sum_of_begins sum_of_ends -> 10^ends_size * sum_of_begins + sum_of_ends) begins ends
