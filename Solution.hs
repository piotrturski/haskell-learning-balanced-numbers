...
newtype Number_of_numbers = Number_of_numbers {get_number_of_numbers :: Integer} deriving (Num, Show)
...
all_numbers_of_size 1 from = fromList [(Sum_of_digits x, (Sum_of_numbers $ toInteger x, Number_of_numbers 1)) | x <- [from..9]]
...
solution n = (get_sum_of_numbers . balanced $ Number_of_digits n) `mod` 3^15